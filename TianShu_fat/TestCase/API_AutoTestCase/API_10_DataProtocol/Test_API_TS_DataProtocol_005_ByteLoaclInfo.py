# -----------------------------------------------------------
#     Test_NO.：Test_API_TS_DataProtocol_005_ByteLoaclInfo
#     Test_Title：根据协议名称（区分大小写）查询某个数据协议格式-字节位
#     Test_Precondition：
#     Test_Step：
#     Author：wangshinan
#     Date：
...
class DataProtocol_ByteLoaclInfo:

    def step(self,*args):
        new_Url = args[0]+args[1]['protoName']
...
@ddt
class Test_API_TS_DataProtocol_005_ByteLoaclInfo(unittest.TestCase):
    ...
    @unpack
    def test_API_TS_DataProtocol_005_ByteLoaclInfo(self,testdata,url,firstURL,attach,hope,number,testNum):
        """
            根据协议名称（区分大小写）查询某个数据协议格式-字节位
        """
        resp1 = DataProtocol_ByteLoaclInfo().step(url,testdata,attach,self.session)

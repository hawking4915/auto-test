# -----------------------------------------------------------
#     Test_NO.：Test_API_TS_DataProtocol_001_Check
#     Test_Title：添加数据协议验重-协议名称(不区分大小写）
#     Test_Precondition：
#     Test_Step：
#     Author：wangshinan
#     Date：
...
from method.excel_process import Get_excel_data,processing_attachData_Dict,processing_attachData_json
from config.ProjectInfo import API_Sheet10,API_testdata_path,username,password
from ddt import ddt,data,unpack
from method.Login import API_Login

class DataProtocol_Check:
...
@ddt
class Test_API_TS_DataProtocol_001_Check(unittest.TestCase):
    ...
    @unpack
    def test_API_TS_DataProtocol_001_Check(self,testdata,url,firstURL,attach,hope,number,testNum):
        """
            添加数据协议验重-协议名称(不区分大小写）
        """
        resp1 = DataProtocol_Check().step(url,testdata,attach,self.session)

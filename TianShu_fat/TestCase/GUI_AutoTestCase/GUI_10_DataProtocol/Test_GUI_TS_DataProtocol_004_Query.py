# -----------------------------------------------------------
#     Test_NO.：Test_GUI_TS_DataProtocol_004_Query
#     Test_Title：数据协议管理：搜索数据协议
#     Test_Precondition：
#     Test_Step：
#     Author：wangshinan
...
class DataProtocol_Query(GUI_Action):

    def step(self,*args):
        self.click_element(By.LINK_TEXT, args[1][0], args[3][0])
        self.click_element(By.XPATH, args[1][1], args[3][1])
        self.sendKeys_element( By.CSS_SELECTOR, args[1][2], args[0][0],args[3][2])
        BeautifulReport.save_img(args[4], "数据协议搜索结果")
        resultList=[]
        for i in args[2]:
            result = self.get_text(By.XPATH, i)
            resultList.append(result)
        test_result = ",".join(resultList)
        return test_result

@ddt
class Test_GUI_TS_DataProtocol_004_Query(unittest.TestCase):
    ...
    @BeautifulReport.add_test_img("数据协议——搜索结果")
    @data(*data_list[sys._getframe().f_code.co_name])
    @unpack
    def test_GUI_TS_DataProtocol_004_Query(self,testdata,clickXpath,hopeXpath,attach,hope,testNum,number):
        """
            数据协议管理：搜索数据协议
        """
        driver = self.driver
        self.newDriver = DataProtocol_Query(driver)
    ...
    def tearDownClass(cls):
        quit(cls.driver)
        moveFiles(abspath(img_path), abspath(img_history_path), 'Test_GUI_TS_DataProtocol_004_Query')

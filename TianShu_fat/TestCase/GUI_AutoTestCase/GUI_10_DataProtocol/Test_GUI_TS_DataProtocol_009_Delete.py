# -----------------------------------------------------------
#     Test_NO.：Test_GUI_TS_DataProtocol_009_Delete
#     Test_Title：数据协议管理：删除数据协议
#     Test_Precondition：
#     Test_Step：
#     Author：wangshinan
...
class DataProtocol_Delete(GUI_Action):
    ...
    def step(self,*args):
        ...
        texts = self.get_texts(By.XPATH, args[1][4])
        xpaths = args[1][5].split("[X]", 1)
        if args[0][0] == "all":
            for i in texts:
                if i == "PDXP" or i == "HLSAP":
                    pass
                else:
                    num = texts.index(i) + 1
                    xpath = xpaths[0] + "[%s]" % str(num) + xpaths[1]
                    self.click_element(By.XPATH, xpath, args[3][5])
                    self.click_element(By.XPATH, args[1][6], "删除数据协议——%s" % i)
                    texts = self.get_texts(By.XPATH, args[1][4])
        else:
            num = texts.index(args[0][0]) + 1
            xpath = xpaths[0] + "[%s]" % str(num) + xpaths[1]
            self.click_element(By.XPATH, xpath, args[3][5])
            self.click_element(By.XPATH, args[1][6], "删除数据协议——%s" %args[0][0])
        BeautifulReport.save_img(args[4], "数据协议——删除结果")
        test_result = self.get_text(By.XPATH, args[2][0])
        return test_result

@ddt
class Test_GUI_TS_DataProtocol_009_Delete(unittest.TestCase):
    ...
    @BeautifulReport.add_test_img("数据协议——删除结果")
    @data(*data_list[sys._getframe().f_code.co_name])
    @unpack
    def test_GUI_TS_DataProtocol_009_Delete(self,testdata,clickXpath,hopeXpath,attach,hope,testNum,number):
        """
            数据协议管理：删除数据协议
        """
        driver = self.driver
        self.newDriver = DataProtocol_Delete(driver)
    ...
    def tearDownClass(cls):
        quit(cls.driver)
        moveFiles(abspath(img_path), abspath(img_history_path), 'Test_GUI_TS_DataProtocol_009_Delete')

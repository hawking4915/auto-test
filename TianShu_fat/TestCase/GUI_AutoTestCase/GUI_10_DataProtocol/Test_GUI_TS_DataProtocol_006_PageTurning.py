# -----------------------------------------------------------
#     Test_NO.：Test_GUI_TS_DataProtocol_006_PageTurning
#     Test_Title：数据协议管理：翻页功能
#     Test_Precondition：
#     Test_Step：
#     Author：wangshinan
...
class DataProtocol_PageTurning(GUI_Action):

    def step(self,*args):
        if args[0][0] == "点击翻页":
            self.click_element(By.XPATH, args[1][0], args[3][0])
        else:
            self.click_element(By.LINK_TEXT, args[1][0], args[3][0])
            self.click_element(By.XPATH, args[1][1], args[3][1])
            self.sendKeys_element(By.XPATH, args[1][2], args[0][0],args[3][2])
            self.ENTER(By.XPATH, args[1][3],args[3][3])
        BeautifulReport.save_img(args[4], "数据协议——翻页结果")
        test_result = self.get_text(By.XPATH, args[2][0])
        return test_result

@ddt
class Test_GUI_TS_DataProtocol_006_PageTurning(unittest.TestCase):
    ...
    @BeautifulReport.add_test_img("数据协议-翻页结果")
    @data(*data_list[sys._getframe().f_code.co_name])
    @unpack
    def test_GUI_TS_DataProtocol_006_PageTurning(self,testdata,clickXpath,hopeXpath,attach,hope,testNum,number):
        """
            数据协议管理：翻页功能
        """
        driver = self.driver
        self.newDriver = DataProtocol_PageTurning(driver)
    ...
    def tearDownClass(cls):
        quit(cls.driver)
        moveFiles(abspath(img_path), abspath(img_history_path), 'Test_GUI_TS_DataProtocol_006_PageTurning')

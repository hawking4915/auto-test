...
def mk_test_name(name, value, index=0):
    ...
    #name：为测试用例的名字,value：为测试数据
    #https://blog.csdn.net/ahilll/article/details/83443569
    index = "{0:0{1}}".format(index + 1, index_len)
    if not is_trivial(value):    #type(value)符合要求：list/tuple
        return "{0}_{1}".format(name, index)
    # try:
    #     value = str(value)
    # except UnicodeEncodeError:#UnicodeEncodeError编码错误
